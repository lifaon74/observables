...
import {
  CreateMappedMemories, MapMemory, ResolveMappedAddress, TMappedMemories, TMappedMemoryAddresses
} from './mapped-memory';
...
function CompactVoxelOctreeByGroupingMaterials(
  ...
  let _address: number | null | undefined = void 0;
  let compactedAddress: number | null;

  for (let i = 0; i < 8; i++) {
    compactedAddress = ReadAddress(memory, subOctreeAddress);

    if (addressesMap !== void 0) {
      compactedAddress = ResolveMappedAddress(compactedAddress, addressesMap);
    }
    ...
    if (IsSubOctreeAddressIndexAVoxelOctreeAddress(memory, address, i)) {
      ...
      if (compactedAddress === null) {
        ...
        WriteSubOctreeAddressIndexAsVoxelMaterial(memory, address, i);
    ...
    if (_address === void 0) {
      _address = compactedAddress;
    } else if ((_address !== null) && (compactedAddress !== _address)) {
      _address = null;
    }
  ...
  return _address as (number | null);
}
