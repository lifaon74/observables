 ...
 * @param inNames
 */
export function aggregateNotifications<TKeysIn extends string, TValue>(inNames: Iterable<TKeysIn>): IPipe<IObserver<INotification<TKeysIn, TValue>>, IObservable<TValue>> {
  ...
  return Pipe.create<TNotification, TValue>((context: TPipeContextBase<TNotification, TValue>) => {
    ...
    return {
      onEmit(notification: TNotification): void {
        if (inNamesSet.has(notification.name)) {
